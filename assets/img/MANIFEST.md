...
| 11 | `why-staff.jpg` | 1200 × 996 | ~1.20:1 | in place | Team photo from the International Furniture Fair booth (matches the milestone in Social Proof). Sits blurred and veiled behind the **heading column, on the right** — the list column (`.why__list`) is visually reordered to the left via CSS `order` rather than by editing the markup, so a screen reader still meets the heading before the list it introduces. The photo's own column is never shared with the list's dense body copy. Fade is vertical, not horizontal: strong protection behind the eyebrow+heading text (stops set against the *actual measured* text position — 28.6% and 53.2% down the box — after two earlier guesses at the fraction under-protected the small eyebrow), relaxing further down where the column was previously just empty ivory, which is where the photo actually gets to show. Blurred deliberately (3px, lighter than an earlier 5px pass): fully sharp put faces at full clarity directly under the headline, which read as an accident rather than a choice. Cropped from `source-images/staff.jpg`. |
