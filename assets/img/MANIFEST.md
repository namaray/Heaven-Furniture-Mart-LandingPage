...
| 11 | `why-chairs.jpg` | 370 × 709 | ~0.52:1 | in place | One carved armchair, crown to feet, sitting blurred and veiled behind the **heading column, on the left** of Why Choose. Cropped from a promo shot of a chair pair + glass table (`source-images/chairs.jpg`) — its "CRAFTED FOR LUXURY LIVING" text and logo overlay were cut out of the saved asset entirely, not just faded behind the veil. Originally cropped to the *pair* of chairs plus the table between them; that wider, landscape crop going into this tall narrow box meant `object-fit: cover` showed only a vertical slice, and the vertical veil (protects the top behind the heading, relaxes lower down) put the ornate carved chair-back inside the heavily-veiled zone and left bare legs and floor in the more-visible zone — technically more visible, but the least interesting part of the photo. Re-cropped tight to one chair (aspect ~0.52, closely matching the box's own ~0.54) so the whole chair survives the cover crop with barely any loss, and both the veiled top and the more-visible bottom now show actual chair. Replaced `why-staff.jpg` (the International Furniture Fair team photo), which is no longer used anywhere on the page — its unprocessed source stays in `source-images/staff.jpg` per the "keep originals" policy below, but the processed crop was deleted since nothing references it now. Same vertical veil gradient and 3px blur as the staff-photo version — those were independent improvements, not specific to the previous image. |
