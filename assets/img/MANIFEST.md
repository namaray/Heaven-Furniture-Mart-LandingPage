...
| 11 | ~~`why-chairs.jpg`~~ | — | — | **removed, not needed** | Why Choose went through two photo treatments (team photo, then a single carved chair) before landing on no photo at all — plain ivory behind the left/right split. Both processed crops are deleted; the raw sources stay in `source-images/staff.jpg` and `source-images/chairs.jpg` per the "keep originals" policy below, in case a photo treatment is wanted here again later. |
